...
def main() -> None:
    ...
    if not hasattr(torch, "is_vulkan_available"):
        fail("torch.is_vulkan_available() is missing in this build")
    try:
        if not torch.is_vulkan_available():
            fail("torch.is_vulkan_available() returned False")
    except Exception as e:
        fail(f"torch.is_vulkan_available() errored: {type(e).__name__}: {e}")

    backend = getattr(getattr(torch, "backends", None), "vulkan", None)
    if backend is not None and hasattr(backend, "is_available"):
        try:
            if not backend.is_available():
                fail("torch.backends.vulkan.is_available() returned False")
        except Exception as e:
            fail(f"torch.backends.vulkan.is_available() errored: {type(e).__name__}: {e}")
