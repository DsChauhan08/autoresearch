...
ensure_setup() {
  ...
  local retryix_url="${RETRYIX_WHEEL_URL:-}"

  if [[ -n "${retryix_url}" ]]; then
    if [[ "${retryix_url}" == *win_amd64.whl* ]]; then
      echo "[igpu-vulkan] ERROR: RETRYIX_WHEEL_URL points to a Windows wheel (${retryix_url})"
      echo "[igpu-vulkan] This Linux path requires a Linux-compatible Vulkan torch wheel."
      return 1
    fi
  fi
